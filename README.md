...
Runthread is a provider-neutral adaptive running training app. Strava is the first likely MVP activity provider, while Garmin direct integration remains an important later target. Private beta readiness is tracked in `docs/private-beta.md`.
...
Without `DATABASE_URL`, server startup uses an in-memory repository store seeded with local demo `athlete-1` and `goal-1` records for the Flutter MVP. If `DATABASE_URL` is set, startup opens a Postgres `database/sql` handle and composes the sqlc-backed repository store. The current-plan read service uses the selected store; the Postgres store implements the richer current-plan snapshot read path for persisted plan weeks, planned workouts, recent imported activities, workout matches, workout results, and adaptation events.
...
These methods are implemented through provider repository interfaces. With Strava OAuth configuration present, `StartProviderConnection` returns an authorization URL and `oauth_ready=true`; the backend callback exchanges the authorization code and stores a token reference. Without Strava configuration, the readiness fallback still creates or reuses a pending provider connection and returns `oauth_ready=false`. Garmin OAuth and auth-backed athlete lookup remain deferred.
...
This repo currently contains documentation, private beta readiness planning, a minimal Go API skeleton, local Postgres infrastructure, provider-neutral domain models, deterministic planning, workout flow helpers, mock Garmin activity normalisation, mock provider import service logic backed by provider-neutral import orchestration, provider import app-service completion orchestration, mock Garmin app-service adapter tests, activity matching, adaptation event generation, in-memory repository boundaries, app-service persistence wiring for plan weeks and first-class planned workouts, application service composition, server configuration and startup storage composition, an initial provider-neutral protobuf API contract, provider connection RPCs, buf generation config, generated protobuf/ConnectRPC Go code, the first thin ConnectRPC handler, implemented in-memory and Postgres-backed read-side RPCs for the Flutter MVP path, Strava OAuth start/callback/token storage wiring, a scaffolded Flutter app with weekly plan, workout detail, history, demo fallback, and read-only completion affordance screens, plain SQL schema migrations, local migration scripts, generated sqlc code, sqlc-backed repositories for the core and provider persistence tables, Postgres store composition layers for core and provider repository interfaces, and end-to-end backend core-loop and mock provider import tests.

It does not yet contain versioned migration tooling, live database integration tests, auth, generated Dart protobuf/ConnectRPC bindings, real Garmin OAuth, production completion writes from mobile, subscriptions, or AI explanation generation.
